          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <div className="p-8">
                ...
                {currentMode === 'login' ? (
                  <LoginForm onSuccess={onAuthSuccess || onClose} onSwitchToRegister={() => setMode('register')} onSwitchToReset={() => setMode('reset')} />
                ) : currentMode === 'register' ? (
                  <RegisterForm onSuccess={onAuthSuccess || onClose} onSwitchToLogin={() => setMode('login')} />
                ) : (
                  <ResetPasswordForm onSuccess={() => setMode('login')} onBack={() => setMode('login')} />
                )}
              </div>
            </motion.div>
          </div>
