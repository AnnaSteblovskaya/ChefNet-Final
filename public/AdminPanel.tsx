  ...
  const submit = async () => {
    ...
    try {
      const token = (window as any).__supabaseSession?.access_token ||
        (() => { try { const r = localStorage.getItem('chefnet-auth-storage'); return r ? JSON.parse(r)?.access_token : null; } catch { return null; } })();
...
export default function AdminPanel({ onExit }: Props) {
  ...
  const checkAdmin = async () => {
    ...
    try {
      ...
      let email = '';
      try {
        const raw = localStorage.getItem('chefnet-auth-storage');
        if (raw) {
  ...
  // Switch favicon to orange admin version
  useEffect(() => {
